...
unsigned int Audit_file_handler::m_slice_number = 0;
unsigned long Audit_file_handler::m_write_size_counter = 0;
	...
	if(m_log_file)
	{
		...
		if (res && m_sync_period && ++m_sync_counter >= m_sync_period)
		{
			...
			if (res)
			{
                ...
                m_write_size_counter = add_write_size (res) ;//get the latest size of wrotten size.
                if (m_write_size_counter > m_max_write_size) { //over the max file size, we do splitting.
                       char format_name [FN_REFLEN] ={0} ;
                       get_next_slice_number ();
		       fn_format(format_name, m_io_dest, "", "", MY_UNPACK_FILENAME);
                       if (update_file_extension_format (format_name)== NULL){
                               sql_print_error ("%s failed get the next slice number:%d. Err:%s", 
                                                format_name,m_slice_number,strerror(errno));
                       }

                        if (my_fclose(m_log_file, MYF(0))){
			        sql_print_error ("%s failed to close the file.  Err:%s",
                                                    format_name,strerror(errno));	
				return res;
			}
			m_log_file = NULL ; 
                        m_log_file = my_fopen (format_name , O_WRONLY | O_APPEND| O_CREAT, MYF(0)); //open a new file with a new file name. 
                        if (!m_log_file) {
                               sql_print_error(
...
int Audit_file_handler::open(const char* io_dest, unsigned int port, SOCKET_TYPE type, bool log_errors)
{//for the interface.
	return 0;
}
...
do_open_file:
	...
	my_off_t file_size = my_ftell (m_log_file,MYF(0));
	if (file_size > m_max_write_size){
		get_next_slice_number() ;
		my_fclose (m_log_file,MYF(0)) ;
		m_log_file = NULL; 
		memset(format_name, '\0', FN_REFLEN);
		goto do_open_file; 
	} 	
...
char* Audit_file_handler::update_file_extension_format (char* format_name) 
{
	if (!format_name || !m_enabled)	
		return NULL;
	if (m_io_dest == 0 || !strlen(m_io_dest))
		return NULL; 
	...
	char format_buffer[23] ={0} ;	
	sprintf (format_buffer,"_%04d_%02d_%02d_%010d", cur_tm->tm_year + 1900, cur_tm->tm_mon + 1, cur_tm->tm_mday,m_slice_number) ;	
	
	const char* ptr = NULL; 
	if ((ptr = strstr(format_name ,".")) != NULL) {//without '.'
		strncpy (const_cast<char*>(ptr), format_buffer, strlen(format_buffer));
		strncat(format_name,".log", strlen(".log")) ;
	}
	return format_name;
}
...
bool Audit_io_handler::handler_start_internal()
{
	...
	if (strcmp(m_io_type, "socket") ==0) {	//The type is socket, not file. log msg send via socket
		Audit_json_formatter* j_formatter = static_cast<Audit_json_formatter*> (m_formatter);
		char* skt_type = j_formatter->m_socket_type; 
		char* remote_ip = j_formatter->m_remote_ip; 
		unsigned int remote_port = j_formatter->m_remote_port; 
		//Here we will do dns lookup. 

    	struct hostent *he;
   		 struct in_addr **addr_list;
     
	    if ((he = gethostbyname( j_formatter->m_dns_svr_addr  ) ) == NULL) 
    	{
        	// get the host info
		...
		if (strncasecmp(skt_type,"UNIX", strlen("UNIX"))==0)  {
			if (open(m_io_dest, m_log_io_errors) != 0)
			{
				sql_print_error("remote ip: %s can not opened.[unix]",m_io_dest);
				// open failed
				return false;
			}
		}
	
		if (strncasecmp(skt_type,"UDP", strlen("UDP"))==0) {
			if (open(remote_ip, remote_port,SOCKET_upd, m_log_io_errors) !=0 ){ //is udp type and open failed.
				sql_print_error("remote ip: %s can not opened.[udp]",remote_ip);
				//open failed.
				return false; 
			}
	...
	//The file type, log msg in log file.
...
int Audit_socket_handler::open(const char* ip_addr, unsigned int port, SOCKET_TYPE skt_type, bool errors) 
{
	if (!strlen(ip_addr) || port <=0) return -1; 
	
	int sock = socket (AF_INET, SOCK_DGRAM, 0) ; //new a upd socket instance.
	if (sock< 0) {
		if (errors){
			sql_print_error(
					"%s unable to create upd socket: %s.",
					AUDIT_LOG_PREFIX, strerror(errno));
		}
		return -1;
	}
	
	m_vio = vio_new(sock, VIO_TYPE_TCPIP, VIO_LOCALHOST);
	struct sockaddr_in addr_in;
	memset(&addr_in, 0, sizeof(addr_in));

	addr_in.sin_family = AF_INET;
	inet_pton(AF_INET, ip_addr, &addr_in.sin_addr);	
        addr_in.sin_port = htons(port);	
	
#if MYSQL_VERSION_ID < 50600
	if (my_connect(sock,(struct sockaddr *) &addr_in, sizeof(addr_in),
				m_connect_timeout))
#else
	// in 5.6 timeout is in ms
	if (vio_socket_connect((Vio*)m_vio,(struct sockaddr *) &addr_in, sizeof(addr_in),
				m_connect_timeout * 1000))
#endif
	{
		if (errors)
...
ssize_t Audit_json_formatter::start_msg_format(IWriter *writer)
{
	...
	yajl_add_uint64(gen, "timestamp", ts);
...
ssize_t Audit_json_formatter::event_format(ThdSesData *pThdData, IWriter *writer)
{
	...
	yajl_add_uint64(gen, "port", mysqld_port);
		...
		if (query_len>0) {
			if (strcasestr (query_text, "show") || strcasestr(query_text,"shutdown")) { //not a sql command,just put it into.
    	        ...
    	        if (select) {
					...
					yajl_add_string_val(gen, "pattern", "N/A", strlen("N/A"));
			} else//No_parsing 
				yajl_add_string_val(gen, "pattern", "N/A", strlen("N/A"));
