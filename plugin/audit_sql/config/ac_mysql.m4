dnl ---------------------------------------------------------------------------
dnl Macro: MYSQL_INSTALL_DIR_TEST
dnl ---------------------------------------------------------------------------

AC_DEFUN([MYSQL_INSTALL_DIR_TEST], [
  AC_MSG_CHECKING([for mysql install dir])
  ac_mysql_install_dir=
  AC_ARG_WITH([mysql-installdir],
        [AS_HELP_STRING([--with-mysql-installdir=PATH], [MySQL install directory where the head files are to be here, need for next compilation steps])],
        [
          ac_mysql_install_dir=`dir_resolve "$withval"`
          if test -d "$ac_mysql_install_dir/" ; then
                MYSQL_INSTALL_DIR="$ac_mysql_install_dir"
                AC_MSG_RESULT([yes: Using $ac_mysql_install_dir])
          else
                AC_MSG_ERROR([invalid MySQL install directory : $ac_mysql_install_dir])
          fi
        ],
        [#if do not set the installation directory, we use the default one.
          #ac_mysql_install_dir=/usr/share/mysql/
          #MYSQL_INSTALL_DIR="$ac_mysql_install_dir"
          #AC_MSG_RESULT([--with-mysql-installindir was not set. Using $ac_mysql_install_dir])
          AC_MSG_ERROR([invalid MySQL install directory, please set its first. : $ac_mysql_install_dir])
        ]
  )
])
...
dnl in libbinlogevents/include.
    ...
    MYSQL_INC="-I$withval/sql -I$withval/libbinlogevents/export -I$withval/libbinlogevents/include -I$withval/include -I$withval/regex -I$withval -I$ac_mysql_install_dir/include -I$ac_mysql_install_dir/libbinlogevents/include"
