...
#define MAX_FILE_SEG_SIZE 2147483648
...
public:
	...
	virtual int open(const char* io_dest, unsigned int port, SOCKET_TYPE type,  bool log_errors) =0;
...
public:
	...
	static const unsigned int max_slice_number = 1024;
	Audit_file_handler() :
		m_sync_period(0), m_log_file(NULL), m_sync_counter(0), m_bufsize(0)
	{
		m_io_type = "file";
	        m_max_write_size = MAX_FILE_SEG_SIZE;// 2 * 1024 * 1024 * 1024. 1GB
		
		init_slice_number_lock() ;
		init_write_size_lokcer();
	}

	virtual ~Audit_file_handler()
	{
		deinit_slice_number_lock () ;
		deinit_write_size_lock();
	}
	...
	int open(const char* io_dest, unsigned int port, SOCKET_TYPE type, bool m_log_errors) ;
	// static void print_sleep(THD *thd, int delay_ms);

	//to update the json log file extension formate.
        //for example: path/audit.log will be change to path/audit_1900_00_00.log.slice_num
	char* update_file_extension_format (char* ) ;

	//get the next slice number
	inline unsigned int get_next_slice_number () {
		lock_slice_number_exclusive();
		m_slice_number ++ ;
		slice_number_unlock(); 
		return (m_slice_number >= max_slice_number)? max_slice_number : m_slice_number; 
	}
	inline unsigned long add_write_size(unsigned int expand) 
	{
		lock_write_size_exclusive() ;
		m_write_size_counter += expand ;
		write_size_unlock ();
		return m_write_size_counter  ;
	}
	inline void reset_write_size () {
		lock_write_size_exclusive();
		m_write_size_counter =0;
		write_size_unlock();
	}
	inline unsigned long get_seg_file_size ()const { return m_max_write_size ;}
	inline void set_seg_file_size (unsigned long max_size) {
		if (max_size>=MAX_FILE_SEG_SIZE)
			m_max_write_size = MAX_FILE_SEG_SIZE; 	
		else
			m_max_write_size = max_size; 
	}
protected: 
	//do lock-init
	inline void init_slice_number_lock (){
		my_rwlock_init (&LOCK_file_slice_number, NULL) ;
	}
	inline void init_write_size_lokcer() {
		my_rwlock_init (&LOCK_write_size_counter,NULL) ;
	}
	inline void deinit_slice_number_lock () {
		rwlock_destroy(&LOCK_file_slice_number);
	}
	inline void deinit_write_size_lock() {
		rwlock_destroy(&LOCK_write_size_counter) ;
	}
	//do lock-shared-lock
	inline void lock_slice_number_shared () {
		rw_rdlock(&LOCK_file_slice_number);
	}
	inline void lock_write_size_shared () {
		rw_rdlock(&LOCK_write_size_counter);
	}
	//do lock-exclusive-lock
	inline void lock_slice_number_exclusive () {
		rw_wrlock (&LOCK_file_slice_number) ;
	}
	inline void lock_write_size_exclusive() {
		rw_wrlock (&LOCK_write_size_counter);
	}
	//do lock-unlock
	inline void slice_number_unlock () {
		rw_unlock(&LOCK_file_slice_number);
	}
	inline void write_size_unlock (){
		rw_unlock(&LOCK_write_size_counter) ;
	}
protected:
	...
	//the slice number, when the size of a log file exceeds over 1G, we will slice its into pieces of.
	static unsigned int m_slice_number; 
	rw_lock_t LOCK_file_slice_number;
	
	//to record how much bytes we have wrotten.	
	rw_lock_t LOCK_write_size_counter;
	static unsigned long m_write_size_counter; 
	
	unsigned long m_max_write_size ;// 2 * 1024 * 1024 * 1024. Max size of a segment file size.
};
...
public:
	...
	int open(const char* ip_addr, unsigned int port, SOCKET_TYPE skt_type, bool log_errors) ; //open a socket with tcpip protocol	
